...
int main(int argc, char *argv[]) {
	...
	double *testdatausingunrollingfor= process_withompusingunrollingfor();
	...
	for(int i = 0; i < GRID_SZ; i++){
		for(int j = 0; j < GRID_SZ; j++){
			if (rightdata[i*GRID_SZ+j]!=testdatausingunrollingfor[i*GRID_SZ+j]) {
				printf("wrong unrollingfor");
				return 0;
			}
		}
	}
...
double *process_withompusingtask() {
		...
		for(i = 0; i < TIMES; i++) {
			sequential_update_withompusingtask( data, olddata, newdata, C, K, dt);
			tmp = olddata;
			olddata = data;
			data = newdata;
			newdata = tmp;
...
void sequential_update_withompusingtask(double *data, double *olddata, double *newdata, double C, double K, double dt ) {
		int i, j;
		double pot;
...
double *process_withompusingunrollingfor() {
		...
		for(i = 0; i < TIMES/3; i++) {
			#pragma omp ordered
			{
				sequential_update_withompusingunrollingfor( data, olddata,  newdata, C, K, dt);
				sequential_update_withompusingunrollingfor( newdata, data , olddata, C, K, dt);
				sequential_update_withompusingunrollingfor( olddata, newdata,  data, C, K, dt);
			}
		}
...
void sequential_update_withompusingunrollingfor(double *data, double *olddata, double *newdata, double C, double K, double dt ) {
		int i, j;
		double pot;
		for( i = 0; i < GRID_SZ; i++) {
			for( j = 0; j < GRID_SZ; j++) {
...
double *process_withompusingfor() {
		...
		for(i = 0; i < TIMES; i++) {
			sequential_update_withompusingfor( data, olddata, newdata, C, K, dt);
			tmp = olddata;
			olddata = data;
			data = newdata;
			newdata = tmp;
...
void sequential_update_withompusingfor(double *data, double *olddata, double *newdata, double C, double K, double dt ) {
		int i, j;
		double pot;
